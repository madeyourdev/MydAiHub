...
| `GROQ_API_KEY` | สมัครฟรีที่ console.groq.com — ใช้สำหรับ Chat AI |
...
| `GROQ_API_KEY` | `ChatModule` — เรียก Groq AI API (จะเปลี่ยนเป็น OpenRouter ในอนาคต) | |
...
| ขั้นตอน | รายละเอียด |
|---|---|
| ตรวจสอบ | ก่อนเรียก Groq API ทุกครั้ง — ถ้า `credits < CREDIT_COST` → ส่ง `400 Insufficient credits` |
...
Railway backend ต้องตั้ง `GROQ_API_KEY` ด้วย — สมัครฟรีที่ console.groq.com (รอเปลี่ยนเป็น `OPENROUTER_API_KEY` ในอนาคต)
